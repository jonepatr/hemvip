...
function VideoPage(
  ...
  var colors = shuffle(["orange", "red", "green", "yellow", "blue", "purple"]);
  i = 0;
  for (var key in this.pageConfig.stimuli) {
    var stimulus = new Stimulus(key, this.pageConfig.stimuli[key]);
...
VideoPage.prototype.render = function (_parent) {
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $(
      ...
      <span class='slider-color-" + conditions[i].color + "'><input type='range' name='" +
        conditions[i].getId() +
        "' class='scales' value='50' min='0' max='100' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>"
    );
